...
class DAT3(nn.Module):
    # smile_vocab = 63 ?? ## I think this should be 62 according to the class Smiles(Alphabets) in utilis.py.
    def __init__(self, embedding_dim, rnn_dim, hidden_dim, graph_dim, dropout_rate, 
                     ...
                     attn_type='dotproduct', vocab=26, smile_vocab = 65, is_pretrain = True,
    ...
    def forward(self, protein, smiles):
        ...
        smiles_lengths = np.array([len(x) for x in smiles]) # (batchsize,)
        #temp = (torch.zeros(batchsize, max(smiles_lengths))*63).long() # why *63?
        temp = (torch.zeros(batchsize, max(smiles_lengths))*65).long()
