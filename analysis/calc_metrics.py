...
from utils import read_smiles_csv, read_smiles_tsv, read_score_csv, get_ledock_score_parallel
...
#     return sum(i<-7 for i in score) / len(score) # using docking output as score

def active_rate(score):
    return sum(i>=1 for i in score) / len(score) # using predictor output as score
...
def success_rate(gen, train, score, n_jobs=1, device='cuda', fp_type='morgan', gen_fps=None, train_fps=None, p=1):
    ...
    that satisfy both similarity < 0.7 and docking score < -7 kcal/mol or binding affinity > 5 pIC50
    """
    if gen_fps is None:
        gen_fps = fingerprints(gen, fp_type=fp_type, n_jobs=n_jobs)
    if train_fps is None:
    ...
    # return sum(i<0.7 and j<-7 for i, j in zip(sim, score)) / len(score) # using docking output as score
    return sum(i<0.7 and j>=1 for i, j in zip(sim, score)) / len(score) # using predictor output as score
...
def main(config):
    ...
    if config.test_path is not None:
        ...
        test = read_smiles_tsv(config.test_path)
